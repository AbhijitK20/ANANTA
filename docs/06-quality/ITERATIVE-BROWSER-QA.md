...
**Scope:** verification of fixes addressing an external QA audit: unified consumer nav (Provider/Operations removed from the top nav, Trips added, "Explore map" renamed "Explore"); a live plan-count badge next to Trips — now on both the home top nav and the shared `BottomNav` (which the experience detail page was missing entirely and now has); a real sort control (ranked / name / price) on Explore; the duplicated "X km from your location" card line removed (the walk-estimate citation is the single proximity statement, score bonus unchanged); event change logs made honest (specific venue → area is shown as the venue diff, current price shown everywhere a changed event appears); "Community reported" label explained; demo booking CTA carries a visible "Demo action · no real booking" badge; CSMT expanded via a shared `stationLabel` helper; quick-filter chips carry a hint that they run real searches.

**Result:** 16/16 automated Chromium checks pass (`scripts/qa-audit-fixes.mjs`), zero console/page errors, no horizontal overflow on home. The harness gained a finally-guarded browser close after the box's Chromium leaked zombie processes and died mid-run twice; 86/86 unit tests and lint pass.
