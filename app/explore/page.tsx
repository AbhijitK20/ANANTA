...
function SearchOverlay({ query, setQuery, applyIntent, city, setCity, category, setCategory, zone, setZone, hiddenGems, setHiddenGems, walkable, setWalkable, free, setFree, bestTime, setBestTime }: { query: string; setQuery: (value: string) => void; applyIntent: (value: string) => void; city: City; setCity: (value: City) => void; category: Category; setCategory: (value: Category) => void; zone: Zone; setZone: (value: Zone) => void; hiddenGems: boolean; setHiddenGems: (value: boolean) => void; walkable: boolean; setWalkable: (value: boolean) => void; free: boolean; setFree: (value: boolean) => void; bestTime: string; setBestTime: (value: string) => void }) { return <form onSubmit={(event) => { event.preventDefault(); applyIntent(query); }} className="absolute left-5 right-5 top-5 sm:left-8 sm:right-8 sm:top-8"><div className="flex items-center gap-3 rounded-xl border border-white/80 bg-white px-4 py-3 shadow-card"><MapPin size={18} className="text-blue" weight="fill" /><input aria-label="Search experiences" className="min-w-0 flex-1 bg-transparent text-sm font-semibold outline-none" placeholder="Food under ₹800 in Mumbai" value={query} onChange={(event) => { setQuery(event.target.value); }} /><button aria-label="Apply search" className="text-muted"><Funnel size={18} /></button></div><div className="mt-3 flex gap-2 overflow-x-auto pb-2"><FilterButton active={city === "Mumbai"} onClick={() => setCity("Mumbai")}>Mumbai</FilterButton><FilterButton active={city === "Navi Mumbai"} onClick={() => setCity("Navi Mumbai")}>Navi Mumbai</FilterButton>{categories.slice(1).map((item) => <FilterButton key={item} active={category === item} onClick={() => setCategory(category === item ? "All" : item)}>{item}</FilterButton>)}<select aria-label="Filter by zone" value={zone} onChange={(event) => setZone(event.target.value as Zone)} className="shrink-0 rounded-lg border border-line bg-white px-3 py-2 text-sm font-bold"><option value="All">All zones</option>{zones.slice(1).map((item) => <option key={item} value={item}>{item}</option>)}</select><select aria-label="Filter by best time" value={bestTime} onChange={(event) => setBestTime(event.target.value)} className="shrink-0 rounded-lg border border-line bg-white px-3 py-2 text-sm font-bold"><option value="Any time">Any time</option>{BEST_TIME_OPTIONS.slice(1).map((item) => <option key={item} value={item}>{item.replace("Best ", "").replace("the ", "")}</option>)}</select></div><div className="flex gap-2 overflow-x-auto pb-2"><FilterButton active={hiddenGems} onClick={() => setHiddenGems(!hiddenGems)}>Hidden gems</FilterButton><FilterButton active={walkable} onClick={() => setWalkable(!walkable)}>Walkable from me</FilterButton><FilterButton active={free} onClick={() => setFree(!free)}>Free entry</FilterButton></div></form>; }
