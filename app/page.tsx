      ...
      <div className="mx-auto min-h-screen max-w-[1480px] bg-white lg:my-5 lg:min-h-[calc(100vh-40px)] lg:rounded-[28px] lg:shadow-card">
        <header className="flex items-center justify-between border-b border-line px-5 py-4 sm:px-8">
          ...
          <nav className="hidden items-center gap-7 text-sm font-semibold text-muted lg:flex">
            <a className="text-ink" href="/">Discover</a><a href="/explore">Explore</a><a href="/events">Events</a><a href="/trips" className="inline-flex items-center gap-1.5">Trips <PlanBadge /></a><a href="/saved">Saved</a><a href="/profile">Profile</a><a href="/privacy">Privacy</a><a href="/terms">Terms</a>
          </nav>
        ...
        </div>          <section className="border-t border-line px-5 py-10 sm:px-8 lg:px-14">
          <SectionHeading eyebrow="Starting soon" title="Happening near you" href="/events" />
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{eventSeed.map((event) => <article key={event.id} className={`border bg-white p-5 transition-colors hover:border-blue ${eventChanges(event).length ? "border-amber bg-amberSoft/40" : "border-line"}`}><div className="h-32 overflow-hidden border border-line bg-[#dfe8e5]"><img src={event.imageUrl} alt="" loading="lazy" className="h-full w-full object-cover" /></div><p className="mt-2 text-[10px] font-semibold text-muted">Photo: {event.imageCredit}</p><div className="mt-3 flex items-center justify-between"><StatusLabel tone="blue">{event.timeLabel}</StatusLabel><CalendarDots size={20} className="text-muted" /></div><h3 className="mt-4 text-lg font-bold tracking-[-0.02em]">{event.name}</h3><p className="mt-2 text-sm leading-6 text-muted">{event.venue} · {event.distance}</p>{eventChanges(event).length > 0 && <p className="mt-2 text-xs leading-5 text-amber">Details changed since the last check. Current price: {event.price}.</p>}<div className="mt-5 flex items-center justify-between text-sm"><span className="font-semibold text-ink">{event.price}</span><a className="font-bold text-blue" href="/events">View event <ArrowRight size={15} className="inline" /></a></div></article>)}</div>
        </section>

        <section className="border-t border-line px-5 py-10 pb-28 sm:px-8 lg:px-14 lg:pb-12"><SectionHeading eyebrow={`Nearest to ${demoUserLocation.label}`} title="Local places worth a look" href="/explore" /><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{nearest.map((place) => { const estimate = estimateFromUser(place.coordinates); return <article key={place.id} className="border border-line bg-white p-5"><div className="h-28 overflow-hidden border border-line bg-[#dfe8e5]"><img src={place.imageUrl} alt="" loading="lazy" className="h-full w-full object-cover" /></div><p className="mt-2 text-[10px] font-semibold text-muted">Photo: {place.imageCredit}</p><div className="mt-3 flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-[0.12em] text-green">{place.category}</span><span className="text-xs font-semibold text-muted">{formatDistance(estimate.km)}</span></div><h3 className="mt-2 font-bold tracking-[-0.02em]">{place.name}</h3><p className="mt-1 text-sm text-muted">{place.area} · {place.duration}</p><div className="mt-4 flex items-center justify-between text-sm"><span className="font-semibold">{place.price}</span><a className="font-bold text-blue" href={`/experience/${place.id}`}>Open</a></div></article>; })}</div></section>
        <BottomNav />
      </div>
    </main>
  );
