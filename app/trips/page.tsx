...
export default function TripsPage() {
  ...
  const places = useMemo(() => getPlannedExperiences(ids, allExperiences), [ids]);
  ...
  const variants = useMemo(() => generatePlanVariants(places, allExperiences.slice(0, 60), budget, availableMinutes, deadline || undefined), [availableMinutes, budget, deadline, places]);
  const closedSuggestions = useMemo(
    () => suggestClosedReplacements({ plan: places, catalog: allExperiences, budget, availableMinutes, deadline: deadline || undefined, availability }),
        ...
        <section className="px-5 pb-28 pt-10 sm:px-8 lg:px-14 lg:pb-14">
          ...
          {places.length ? (
            <>
              ...
              <div className={`mt-6 border p-4 ${evaluation.feasible ? "border-green bg-greenSoft" : "border-amber bg-amberSoft"}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {evaluation.hasWeatherWarning && <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center"><p className="text-sm font-semibold text-amber">One selected experience is weather dependent.</p><button onClick={() => setWeatherAlternative(allExperiences.find((item) => item.statusTone !== "amber" && !ids.includes(item.id))?.id ?? null)} className="text-left text-sm font-bold text-blue hover:underline">Find an indoor alternative</button></div>}
                  </div>
                </div>
              </div>
              ...
              {weatherAlternative && <AlternativeCard alternativeId={weatherAlternative} currentIds={ids} onClose={() => setWeatherAlternative(null)} onApply={() => { const next = ids.filter((id) => allExperiences.find((item) => item.id === id)?.statusTone !== "amber"); writePlan([...next, weatherAlternative]); setWeatherAlternative(null); }} />}
...
function PlanTimeline({ places }: { places: Experience[] }) {
...
function AlternativeCard({ alternativeId, currentIds, onClose, onApply }: { alternativeId: string; currentIds: string[]; onClose: () => void; onApply: () => void }) {
  const alternative = allExperiences.find((item) => item.id === alternativeId);
